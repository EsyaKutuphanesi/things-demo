...
function checkProgress(mastery){
    $.getJSON( "/game_data", function( data ) {
        ...
        $.each(data["mastery"], function(key, val){
            ...
            if(mastery == key){
                var path = "/static/images/game/" + key + "_" + (val["level"] + 1) + ".png";
                mastered = counter;
            }
            else
                var path = "/static/images/game/" + key + "_" + val["level"] + ".png";
            imgs.push(path)
            counter++;
        });
...
function gozuAcik(clear){
  ...
  if(clear)
    clearLayer('layer2');
  addImage('layer2', '/static/images/game/kitap_kurdu_gozu_acik.png',110, 130, 91, 167);
...
function gozuKapali(clear){
    ...
    if(clear)
        clearLayer('layer2');
    addImage('layer2', '/static/images/game/kitap_kurdu_gozu_kapali.png',110, 130, 91, 167);
    //clearInterval(interval);
